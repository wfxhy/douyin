...
public class AgentUserController extends BaseController
{
    ...
    @PreAuthorize("@ss.hasPermi('agent:user:export')")
    @PostMapping("/export")
    ...
    @PreAuthorize("@ss.hasPermi('agent:user:import')")
     ...
     * 新增用户
     */
    @PreAuthorize("@ss.hasPermi('agent:user:add')")
     ...
     * 重置密码
     */
    @PreAuthorize("@ss.hasPermi('agent:user:resetPwd')")
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @PutMapping("/resetPwd")
